"""Swissmedic 藥品資料載入與過濾

載入瑞士 Swissmedic 藥品註冊資料。
"""
...
def load_fda_drugs(filepath: Optional[Path] = None) -> pd.DataFrame:
    """載入瑞士 Swissmedic 藥品資料

    Args:
        filepath: JSON 檔案路徑，預設為 data/raw/ch_fda_drugs.json

    Returns:
        包含所有藥品的 DataFrame

    Raises:
        FileNotFoundError: 找不到 Swissmedic 資料檔案時，提供下載指引
    """
    if filepath is None:
        # loader.py -> data -> chtxgnn -> src -> CHTxGNN
        filepath = Path(__file__).parent.parent.parent.parent / "data" / "raw" / "ch_fda_drugs.json"

    if not filepath.exists():
        raise FileNotFoundError(
            f"找不到 Swissmedic 藥品資料: {filepath}\n"
            f"請先執行以下步驟：\n"
            f"1. 執行: uv run python scripts/process_fda_data.py\n"
            f"   (腳本會從 Swissmedic 下載 XLSX 資料)"
        )
